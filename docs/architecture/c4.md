...
| `rules.py` | Facade over the rule submodules (below); 26 deterministic rules + waiver handling | `Rule`, `Finding`, `evaluate`, `evaluate_tree` |
...
| `sarif.py` | Pure projection: serialized findings → a SARIF 2.1.0 log (CP-6) | `to_sarif` |
| `report.py` | Pure projection: a saved findings envelope → gate status, GitHub annotations, job summary, CI outputs (CP-GA) | `parse_envelope`, `status_of`, `to_annotations`, `to_outputs` |
...
flowchart TD
    ...
    rules --> rules_speckit["rules_speckit.py<br/>S001-S004"]
...
`rules_generic.py` covers G001-G009, `rules_harness.py` covers H001-H006,
`rules_upstream.py` covers U001-U005, `rules_speckit.py` covers S001-S004,
