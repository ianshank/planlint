...
| `tools/*` | CI gates (coverage, secrets, diff, thresholds) + generators (`render_mermaid.py`, `render_rule_catalog.py`, `render_plugin_manifests.py`) + the matcher-accuracy scorer (`matcher_accuracy.py`, a report whose gate is `tests/test_matcher_accuracy.py`). Stdlib-only, so they run in a bare CI runner before anything is installed; shared helpers in `_common.py`. Every root they read resolves at call time through a `None` sentinel rather than being bound into a signature default, which is what makes a gate testable against a fixture tree instead of only against its own checkout | standalone scripts |
