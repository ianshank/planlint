...
4. **Add a deterministic test** for the new rule — a fixture that violates it and an assertion the rule fires on exactly that violation (this repo's own stated test philosophy: "a linter that never fails is a decoration," from `tests/test_graft_rules.py`'s module docstring).
